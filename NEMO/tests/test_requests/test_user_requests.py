...
class UserRequestsTestCase(NEMOTestCaseMixin, TestCase):
    ...
    def test_user_requests_staff_assistance_enabled(self):
        self.login_as(self.user)
        # Disable others, enable staff assistance
        from NEMO.models import Customization
        from NEMO.views.customization import CustomizationBase

        Customization.objects.update_or_create(name="staff_assistance_requests_enabled", defaults={"value": "enabled"})
        CustomizationBase.invalidate_cache()
    ...
    def test_user_requests_training_default(self):
        self.login_as(self.user)
        # Disable all others
        response = self.client.get(reverse("user_requests"))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["tab"], "training")
